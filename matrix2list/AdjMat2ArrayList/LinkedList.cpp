...
Payload* LinkedList::dequeueLIFO()
{
	...
	else
	{
		...
		int pRoomNumber = payP->getRoomNumber();
		...
		if(temp->prev)
		{
			...
			//free(temp->next);
			int tRoomNumber = payP->getRoomNumber();
...
void LinkedList::printHistory1()
{
	...
	else
	{
		...
		while(temp->next)
		{
			room =temp->payP->getRoomNumber();
			treasureSubtotal+= temp->payP->getTreasure();
			printf("The room was %d, and the treasure subtotal was %f.\n", room, treasureSubtotal);
			temp=(LLNode*)temp->next;
...
bool LinkedList::isMember(Payload* p)
{
	bool answer = false;
	if(begin1->payP ==(Payload*)0)
	{
	...
	else
	{
		...
		while(temp->next)
		{
			if(temp->payP->getNodeNumber() == p->getNodeNumber())
			{
				answer = true;
			}
			temp=(LLNode*)temp->next;
